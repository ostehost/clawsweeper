...
function recordCommandMutationOutcome(
  ...
  recordCommandEvent(command, ACTION_EVENT_TYPES.commandMutation, {
    ...
    uncertainMutation: options.outcome === "unknown",
    ...(options.component ? { component: options.component } : {}),
    eventIdentity: {
      kind: options.kind,
...
export function recoverOpenCommandActionEvents(): number {
  return interruptOpenWorkflowActionEvents(commandActionLedgerRoot(), {
    reasonCode: ACTION_EVENT_REASON_CODES.workflowFailed,
    eventTypes: [ACTION_EVENT_TYPES.commandMutation],
  });
}
...
function recordCommandEvent(
  ...
  if (options.mutation) chain.mutationObserved = true;
  if (options.uncertainMutation) chain.uncertainMutationObserved = true;
}
