...
export const WORKER_CLUSTER_JOB_NAME = "Plan and review cluster";
export const WORKER_EXECUTE_JOB_NAME = "Prepare reviewed cluster actions without write credentials";
export const WORKER_PUBLISH_JOB_NAME = "Publish reviewed cluster actions";
...
export function resolveWorkerPublicationCohort(input: {
  ...
  const publishJob = selectLatestJobResult(
    input.jobInventories,
  ...
  return {
    ...
    publishLedger: selectExactAttemptArtifact(
      input.artifactPages,
      `clawsweeper-repair-worker-action-ledger-publish-${runId}`,
      publishJob.attempt,
    ),
  };
}
