...
import {
  ...
  recoverOpenCommandActionEvents,
...
export async function finalizeCommandActionLedgerManifest(
  ...
  const eventPaths = commandShards
    .map((shard) => portableCommandActionEventPath(shard.relativePath))
    .sort();
...
export function parseCommandActionLedgerManifest(
  ...
  if (eventPaths.some((relativePath) => relativePath.includes("\\"))) {
    throw new Error("command action ledger manifest paths must use POSIX separators");
  }
...
function portableCommandActionEventPath(relativePath: string): string {
