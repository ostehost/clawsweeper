...
function recordExecutionReportArtifactSafely(reportPath: string) {
  try {
    ...
    console.error(
      `[action-ledger] failed to record repair execution report: ${
        receiptError instanceof Error ? receiptError.message : String(receiptError)
      }`,
    );
  }
...
const requiresPreparedPublication =
  plannedFixActions.length !== 0 && !NON_EXECUTABLE_REPAIR_STRATEGIES.has(repairStrategy);
...
if (validateNoPublicationOnly || publishNoPublication) {
  if (requiresPreparedPublication) {
    throw new Error(
  ...
  if (NON_EXECUTABLE_REPAIR_STRATEGIES.has(repairStrategy)) {
    recordNonExecutableRepair(report, repairStrategy);
  } else {
    report.status = "skipped";
    report.reason = "immutable cluster result has no executable planned fix actions";
    report.actions.push({
      action: "execute_fix",
      status: "skipped",
...
function recordNonExecutableRepair(report: LooseRecord, repairStrategy: string) {
  report.status = "skipped";
  report.reason = `fix_artifact.repair_strategy ${repairStrategy} is not executable`;
  if (repairStrategy === "needs_human" && Array.isArray(result.needs_human)) {
    report.needs_human = uniqueStrings(result.needs_human.map(String));
  }
  report.actions.push({
    action: "execute_fix",
    status: "skipped",
      ...
      const errorDetail = timedOut
        ...
        : codexResult.error
          ...
          : codexResult.status !== 0
            ...
            : "";
      if (errorDetail) codexAction.fail(new Error(errorDetail));
      else codexAction.complete();
...
function compactReplacementHistory({
  ...
  const compaction = compactGeneratedBranchHistory({
    ...
    commitCommand: (args) => runTrustedGitMutation(args, targetDir),
  });
...
function pushRecoverableBranch({ targetDir, branch, commit }: LooseRecord) {
  ...
  runDirectRepairMutation(
    "branch_push",
    { repo: result.repo, branch, remoteSha, targetRef, head: publicationCommit },
...
function writeReport(report: LooseRecord, resultPath: string) {
  ...
  recordExecutionReportArtifactSafely(reportPath);
