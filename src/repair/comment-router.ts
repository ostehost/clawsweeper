...
function classifyAutomergePass(
  ...
  if (String(issue.state ?? "").toLowerCase() !== "open") {
    const replay = classifyDispatchedMergeClaimReplay(command, pull);
...
function classifyDispatchedMergeClaimReplay(
  command: LooseRecord,
  pull: LooseRecord,
): LooseRecord | null {
  if (!pull) return null;
  ...
  try {
    ...
    return {
      ...command,
      status: "waiting",
      reason: `durable exact-head merge claim could not be read: ${compactGhError(error)}`,
      actions: [],
    };
  }
  if (claim.status === "blocked" || claim.status === "unknown") {
    return {
      ...command,
      status: claim.status === "unknown" ? "waiting" : "blocked",
      reason: claim.reason,
      actions: [],
    };
  }
  if (claim.status !== "existing" || claim.dispatched !== true) return null;
  return {
    ...command,
    status: "ready",
    reason: "rechecking a durably dispatched exact-head merge claim against live GitHub state",
    actions: [
      { action: "merge", status: execute ? "pending" : "planned" },
      { action: "comment", status: execute ? "pending" : "planned" },
    ],
...
function executeAutomerge(command: LooseRecord): LooseRecord {
  ...
  let priorClaim: ReturnType<typeof inspectAutomergeMergeClaim>;
  try {
    priorClaim = inspectAutomergeMergeClaim(command);
  } catch (error) {
    return {
      action: "merge",
      status: "waiting",
      reason: `exact-head merge claim could not be inspected: ${compactGhError(error)}`,
      merge_method: "squash",
    };
  }
  if (
    priorClaim.status === "blocked" ||
    priorClaim.status === "unknown" ||
    (priorClaim.status === "existing" && priorClaim.dispatched === true)
  ) {
    return reconcileClaimedAutomergeRequest(command, priorClaim, waitedMs, transientObservations);
  }
  const stoppedReason = repairLoopStoppedReason(command);
  if (stoppedReason) {
  ...
  const finalDispatchSafetyBlock = (): {
    ...
    if (strictBaseBindingBlock) {
      return { reason: strictBaseBindingBlock, retryable: false };
    }
    let postPolicyState;
    let terminalState;
    try {
        ...
        reconcile: ({ result: commandResult, error: commandError }) => {
          try {
            ...
            return {
              ...
              snapshot,
              snapshot_error: "",
              confirmation: confirmDualViewAutomergeEffectSnapshot(
                snapshot,
                command.expected_head_sha,
...
function observeExistingAutomergeEffect(command: LooseRecord, view: LooseRecord) {
  ...
  const squashCommitProof = durableExpectedMessage
    ...
    : undefined;
  const confirmation = confirmDualViewAutomergeEffectSnapshot(
    snapshot,
    command.expected_head_sha,
    mergeOwned
      ? squashCommitProof
  ...
  if (!confirmation.mergedAt) {
    return {
      ...
      reason:
        ...
        "merged pull request state is not confirmed by both GitHub pull request views",
      merge_method: "squash",
    };
  }
  return {
...
function confirmDualViewAutomergeEffectSnapshot(
  snapshot: LooseRecord,
  expectedHeadSha: JsonValue,
  proof: AutomergeEffectProof = {},
) {
  const effect = confirmAutomergeEffectSnapshot(snapshot, expectedHeadSha, proof);
  const pull = snapshot.pull ?? {};
  const view = snapshot.view ?? {};
  const hasMergeSignal = Boolean(
    pull.merged_at || view.mergedAt || String(view.state ?? "").toUpperCase() === "MERGED",
  );
  if (!hasMergeSignal) return effect;
  const confirmation = confirmAlreadyMergedPullRequest({
    expectedHeadSha,
    pull,
    view,
  });
  if (!confirmation) return effect;
  ...
  if (effect.block) return effect;
  return {
    ...effect,
    mergedAt: confirmation.mergedAt,
    mergeCommitSha: confirmation.mergeCommitSha,
  };
}
