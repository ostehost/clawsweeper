...
function finalizeFixPr(action: LooseRecord) {
  ...
  for (;;) {
    ...
    if (
      restMergeConfirmed !== graphqlMergeConfirmed ||
      (Boolean(mergedAt) && !(restMergeConfirmed && graphqlMergeConfirmed)) ||
      (pull.merged === true) !== (restMergeConfirmed && graphqlMergeConfirmed) ||
...
function confirmPostFlightMergeSnapshot(
  ...
  if (confirmation?.status === "waiting") {
    return {
      mergedAt: null,
      block: "",
      pendingReason: "merge state is not yet confirmed by both GitHub pull request views",
    };
  }
  if (confirmation && confirmation.status !== "executed") {
    return { mergedAt: null, block: confirmation.reason, pendingReason: "" };
  }
...
function postFlightMergeRetryBlock({
  ...
  pull,
  view,
  enforceStrictBase = true,
}: {
  action: LooseRecord;
  number: number;
  pull: LooseRecord;
  view: LooseRecord;
  enforceStrictBase?: boolean;
}) {
