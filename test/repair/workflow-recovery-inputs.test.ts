...
test("repair retries reuse immutable Action inputs while preserving workflow isolation", () => {
  ...
  const tokenIndex = workflow.indexOf("- name: Create exact-repository GitHub App token");
  ...
  assert.doesNotMatch(workflow, /args=.*inputs\.model/);
  ...
  assert.match(selfHealWorkflow, /CLAWSWEEPER_WORKER_RUNNER: blacksmith-4vcpu-ubuntu-2404/);
  assert.match(selfHealWorkflow, /CLAWSWEEPER_EXECUTION_RUNNER: blacksmith-16vcpu-ubuntu-2404/);
  assert.doesNotMatch(selfHealWorkflow, /CLAWSWEEPER_WORKER_RUNNER:.*inputs\.runner/);
  assert.doesNotMatch(selfHealWorkflow, /CLAWSWEEPER_EXECUTION_RUNNER:.*inputs\.execution_runner/);
});
