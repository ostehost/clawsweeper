...
test("cluster worker seals the immutable job into the reviewed worker artifacts", () => {
  const workflow = parse(fs.readFileSync(workerWorkflowPath, "utf8"));
  const sealStep = workflow.jobs.cluster.steps.find(
    (step: { name?: string }) => step.name === "Seal immutable source in reviewed worker artifacts",
  );
  ...
  const stagedRoot = path.join(root, "staged-worker");
  const immutableBytes = Buffer.from("sealed immutable job bytes\n", "utf8");
  const digest = createHash("sha256").update(immutableBytes).digest("hex");
  fs.mkdirSync(path.dirname(immutablePath), { recursive: true });
  fs.writeFileSync(immutablePath, immutableBytes);
  fs.mkdirSync(stagedRoot, { recursive: true });
  fs.writeFileSync(path.join(stagedRoot, "result.json"), "{}\n");
  fs.writeFileSync(path.join(stagedRoot, "cluster-plan.json"), "{}\n");
  ...
  try {
    ...
    assert.equal(child.status, 0, child.stderr);
    assert.deepEqual(fs.readFileSync(path.join(stagedRoot, "source-job.md")), immutableBytes);
    const identity = JSON.parse(fs.readFileSync(path.join(stagedRoot, "source-job.json"), "utf8"));
...
test("cluster worker uploads only the exact reviewed and sealed result files", () => {
  const workflow = parse(fs.readFileSync(workerWorkflowPath, "utf8"));
  const steps = workflow.jobs.cluster.steps as Array<{
    name?: string;
    with?: { path?: string };
  }>;
  const expectedPaths = [
    "${{ runner.temp }}/clawsweeper-worker-stage/result.json",
    "${{ runner.temp }}/clawsweeper-worker-stage/cluster-plan.json",
    "${{ runner.temp }}/clawsweeper-worker-stage/source-job.md",
    "${{ runner.temp }}/clawsweeper-worker-stage/source-job.json",
  ];

  for (const name of ["Upload worker transfer artifacts", "Upload final worker artifacts"]) {
    const upload = steps.find((step) => step.name === name);
    assert.ok(upload, name);
    assert.equal(typeof upload.with?.path, "string", name);
    const paths = upload.with.path
      .split("\n")
      .map((value) => value.trim())
      .filter(Boolean);
    assert.deepEqual(paths, expectedPaths, name);
  }
});

test("execute worker verifies every immutable source seal and rejects invalid provenance", () => {
  const workflow = parse(fs.readFileSync(workerWorkflowPath, "utf8"));
  const verifyStep = workflow.jobs.execute.steps.find(
    (step: { name?: string }) => step.name === "Verify immutable source seal in worker artifacts",
  );
  assert.equal(typeof verifyStep?.run, "string");

  for (const scenario of ["valid", "missing", "tampered", "identity", "symlink"] as const) {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), `clawsweeper-source-${scenario}-`));
    const jobPath = "jobs/openclaw/inbox/cluster-openclaw-openclaw-sealed.md";
    const immutableRoot = path.join(root, ".clawsweeper-repair", "immutable-state");
    const immutablePath = path.join(immutableRoot, jobPath);
    const runDir = path.join(root, ".clawsweeper-repair", "runs", "cluster-a");
    const immutableBytes = Buffer.from("sealed immutable job bytes\n", "utf8");
    const digest = createHash("sha256").update(immutableBytes).digest("hex");
    fs.mkdirSync(path.dirname(immutablePath), { recursive: true });
    fs.writeFileSync(immutablePath, immutableBytes);
    ...
    fs.writeFileSync(path.join(runDir, "cluster-plan.json"), "{}\n");
    fs.writeFileSync(path.join(runDir, "source-job.md"), immutableBytes);
    fs.writeFileSync(
    ...
    if (scenario === "missing") {
      fs.rmSync(path.join(runDir, "source-job.json"));
    } else if (scenario === "tampered") {
      fs.appendFileSync(path.join(runDir, "source-job.md"), "tampered\n");
    } else if (scenario === "identity") {
      const identity = JSON.parse(fs.readFileSync(path.join(runDir, "source-job.json"), "utf8"));
      identity.job_sha256 = "0".repeat(64);
      fs.writeFileSync(path.join(runDir, "source-job.json"), `${JSON.stringify(identity)}\n`);
    } else if (scenario === "symlink") {
      fs.rmSync(path.join(runDir, "source-job.md"));
      fs.symlinkSync(immutablePath, path.join(runDir, "source-job.md"));
    }

    try {
      const child = spawnSync("bash", ["-c", verifyStep.run], {
        cwd: root,
        encoding: "utf8",
        env: {
          ...process.env,
      ...
      if (scenario === "valid") {
        ...
        assert.notEqual(child.status, 0, `${scenario} provenance unexpectedly passed`);
      }
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
...
test("publisher independently verifies the immutable source seal before minting mutation authority", () => {
  const workflow = parse(fs.readFileSync(workerWorkflowPath, "utf8"));
  ...
  const groundTruth = steps.find((step) => step.name === "Resolve one immutable cluster result");
  const mutationToken = steps.find(
    (step) => step.name === "Create narrow no-publication mutation token after validation",
  );

  assert.equal(typeof groundTruth?.run, "string");
  assert.equal(groundTruth?.env?.SOURCE_JOB_PATH, "${{ inputs.job }}");
  assert.equal(groundTruth?.env?.STATE_REVISION, "${{ needs.cluster.outputs.state_revision }}");
  assert.equal(groundTruth?.env?.JOB_SHA256, "${{ needs.cluster.outputs.job_sha256 }}");
  assert.match(groundTruth.run, /\["cluster-plan\.json", "source-job\.md", "source-job\.json"\]/);
  assert.match(groundTruth.run, /sealedSource\.equals\(immutableSource\)/);
  assert.match(groundTruth.run, /sourceDigest !== process\.env\.JOB_SHA256/);
  assert.match(groundTruth.run, /sourceIdentity\.source_job !== process\.env\.SOURCE_JOB_PATH/);
  assert.match(groundTruth.run, /sourceIdentity\.state_revision !== process\.env\.STATE_REVISION/);
  assert.match(groundTruth.run, /sourceIdentity\.job_sha256 !== process\.env\.JOB_SHA256/);
  assert.ok(steps.indexOf(groundTruth!) < steps.indexOf(mutationToken!));
});
...
test("commit finding and worker workflows bind immutable review inputs", () => {
  ...
  const intakeWorkflow = parse(intake);
  const intakeSteps = intakeWorkflow.jobs.intake.steps as Array<{
    env?: Record<string, string>;
    if?: string;
    name?: string;
    run?: string;
  }>;
  const bindPublishedJob = intakeSteps.find((step) => step.name === "Bind immutable published job");
  const dispatchWorker = intakeSteps.find(
    (step) => step.name === "Dispatch isolated repair worker",
  );
  ...
  assert.ok(bindPublishedJob && typeof bindPublishedJob.run === "string");
  assert.match(
    bindPublishedJob.run,
  ...
  assert.match(bindPublishedJob.run, /job_sha256="\$\(sha256sum "\$immutable_job"/);
  assert.match(bindPublishedJob.run, /cp "\$immutable_job" "\$JOB_PATH"/);
  assert.ok(dispatchWorker && typeof dispatchWorker.run === "string");
  assert.equal(
    dispatchWorker.if,
    "${{ steps.prepare.outputs.should_repair == 'true' && steps.published-job.outcome == 'success' }}",
  );
  assert.equal(
    dispatchWorker.env?.STATE_REVISION,
    "${{ steps.published-job.outputs.state_revision }}",
  );
  assert.equal(dispatchWorker.env?.JOB_SHA256, "${{ steps.published-job.outputs.job_sha256 }}");
  assert.match(dispatchWorker.run, /pnpm run repair:dispatch -- "\$JOB_PATH"/);
  assert.match(dispatchWorker.run, /--state-revision "\$STATE_REVISION"/);
  assert.match(dispatchWorker.run, /--job-sha256 "\$JOB_SHA256"/);
  assert.ok(intakeSteps.indexOf(bindPublishedJob) < intakeSteps.indexOf(dispatchWorker));
...
test("all production dispatch callers require exact published job revisions", () => {
  ...
  assert.match(
    createJob,
    /spawnGitBytes\(\["show", `\$\{revision\}:\$\{relativePath\}`\], path\.resolve\(stateRoot\)\)/,
  );
  assert.match(createJob, /resolveSpawnCommand\("git", args, \{ cwd, env: process\.env \}\)/);
  assert.match(createJob, /spawnSync\(invocation\.command, invocation\.args/);
  assert.doesNotMatch(createJob, /spawnSync\(["']git["']/);
