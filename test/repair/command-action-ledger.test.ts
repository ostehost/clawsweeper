...
test("command failure reflects only successfully recorded prior mutations", async () => {
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "command-claim-failure-")));
  const outputRoot = path.join(root, "output");
  fs.mkdirSync(outputRoot);
  const previous = { ...process.env };
  Object.assign(process.env, {
    ...
    CLAWSWEEPER_ACTION_LEDGER_ROOT: root,
    CLAWSWEEPER_ACTION_LEDGER_OUTPUT_ROOT: outputRoot,
    CLAWSWEEPER_ACTION_LEDGER_INVOCATION: "claim-failure",
    GITHUB_REPOSITORY: "openclaw/clawsweeper",
  ...
  try {
    const claimed = syntheticCommand("5".repeat(40), "claim-then-fail");
    claimed.status = "claimed";
    recordCommandClaimed(claimed);
    recordCommandFailure(claimed, new Error("failure after claim"));

    const unrecorded = {
      ...syntheticCommand("4".repeat(40), "disabled-mutation"),
      issue_number: 43,
      idempotency_key: "repair-loop-label-sweep:openclaw/openclaw:automerge:43",
    };
    process.env.CLAWSWEEPER_ACTION_LEDGER_DISABLED = "1";
    runCommandMutation(unrecorded, {
      kind: "label_add",
      identity: { repository: unrecorded.repo, number: unrecorded.issue_number },
      operation: () => "accepted",
    });
    delete process.env.CLAWSWEEPER_ACTION_LEDGER_DISABLED;
    recordCommandFailure(unrecorded, new Error("failure after unrecorded mutation"));
    await flushCommandActionEvents();

    const events = readEvents(outputRoot);
    const claimedFailure = events.find(
      (event) => event.event_type === "command.failed" && event.subject.number === 42,
    );
    assert.equal(claimedFailure?.action.mutation, true);
    assert.equal(claimedFailure?.action.retryable, false);
    assert.equal(claimedFailure?.attributes.completion_reason, "mutation_observed");

    const unrecordedFailure = events.find(
      (event) => event.event_type === "command.failed" && event.subject.number === 43,
    );
    assert.equal(unrecordedFailure?.action.mutation, false);
    assert.equal(unrecordedFailure?.action.retryable, false);
    assert.equal(unrecordedFailure?.attributes.completion_reason, "failed");
  } finally {
    for (const key of Object.keys(process.env)) {
      if (!(key in previous)) delete process.env[key];
    }
