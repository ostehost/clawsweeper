...
test("repair worker jobs upload current-attempt ledgers for the trusted publisher", () => {
  ...
  const execute = worker.slice(worker.indexOf("\n  execute:"), worker.indexOf("\n  publish:"));
  const publish = worker.slice(worker.indexOf("\n  publish:"));
  const clusterWorker = cluster.slice(
    cluster.indexOf("- name: Run isolated worker and stage exact ground truth"),
    cluster.indexOf("- name: Review staged worker result"),
  );
  ...
  assert.match(
    cluster,
    /id: cluster-setup-pnpm\n\s+if: \$\{\{ steps\.check_job\.outputs\.job_exists == '1' \}\}\n\s+with:/,
  );
  assert.match(
    cluster,
  ...
  assert.match(
    cluster,
    /name: Register steerable Action session[\s\S]*register "\$JOB_PATH" --skip-repair-receipt/,
  );
  assert.match(
    cluster,
  ...
  assert.match(clusterWorker, /dist\/repair\/run-worker\.js/);
  assert.ok(
  ...
  assert.equal(
    (publish.match(/uses: \.\/\.github\/actions\/setup-action-ledger/g) ?? []).length,
    1,
  );
  ...
  assert.match(
    publish,
    /clawsweeper-repair-worker-action-ledger-publish-\$\{\{ github\.run_id \}\}-\$\{\{ github\.run_attempt \}\}/,
  );
  assert.ok(
    publish.indexOf("uses: ./.github/actions/setup-action-ledger") <
      publish.indexOf("Create narrow no-publication mutation token after validation"),
  );
  assert.ok(
    publish.indexOf("- name: Record work failure") <
      publish.indexOf("- name: Finalize publication repair action ledger"),
  );
  ...
  assert.match(publisher, /publish_job_attempt: String\(publishJob\.attempt\)/);
  assert.match(publisher, /Download cluster action ledger/);
  assert.match(publisher, /Download execution action ledger/);
  assert.match(publisher, /Download publication action ledger/);
  ...
  assert.match(
    publisher,
    /verify_lane publish publish "\$PUBLISH_JOB_RESULT" "\$PUBLISH_LEDGER_FOUND" "\$PUBLISH_JOB_ATTEMPT" true/,
  );
  assert.match(
    publisher,
  ...
  assert.match(
    publisher,
    /import_worker_lane publish publish "\$PUBLISH_JOB_RESULT" "\$PUBLISH_JOB_ATTEMPT" true/,
  );
...
test("commit review and notification workflows publish their operation receipts", () => {
  ...
  assert.match(publisher, /selectLatestJobResult/);
  assert.match(publisher, /selectExactAttemptArtifact/);
  assert.match(
    publisher,
    /artifact-ids: \$\{\{ steps\.commit-report-cohort\.outputs\.review_artifact_ids \}\}/,
  );
  assert.match(
    publisher,
    /name: \$\{\{ steps\.commit-report-cohort\.outputs\.skipped_artifact_name \}\}/,
  );
  assert.doesNotMatch(
    publisher,
  ...
  assert.match(publisher, /Resolve exact commit report artifact cohort/);
  assert.match(publisher, /Verify exact commit review bundles/);
  ...
  assert.match(publisher, /Publish immutable selected commit review action ledger/);
  ...
  assert.ok(
    publisher.indexOf("- name: Resolve exact commit report artifact cohort") <
      publisher.indexOf("- name: Verify exact commit review bundles"),
  );
  assert.ok(
    publisher.indexOf("- name: Verify exact commit review bundles") <
  ...
  assert.ok(
    publisher.indexOf("- name: Finalize commit publication action ledger") <
      publisher.indexOf("- name: Publish immutable selected commit review action ledger"),
  );
  assert.match(
    publisher,
...
test("result and finalizer workflows publish their repair operation receipts", () => {
  ...
  assert.match(results, /import_worker_lane publish publish/);
...
test("repair and commit publishers require canonical exact manifests", () => {
  ...
  assert.match(
    commit,
    /Resolve exact commit report artifact cohort[\s\S]*Verify exact commit review bundles[\s\S]*repair:action-ledger -- verify[\s\S]*Commit reports[\s\S]*repair:action-ledger -- publish/,
  );
});
