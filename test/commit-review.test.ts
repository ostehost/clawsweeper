...
test("commit report publication binds every SHA to its latest actual review job attempt", () => {
  const workflow = fs.readFileSync(".github/workflows/commit-review.yml", "utf8");
  ...
  assert.match(
    publisher,
    /selectLatestJobResult\(\s*jobInventories,\s*`Review commit \$\{sha\}`,\s*runAttempt,/,
  );
  assert.match(publisher, /if \(job\.result !== "success"\)/);
  assert.match(publisher, /`commit-review-\$\{sha\}-\$\{runId\}`/);
  assert.match(publisher, /successful Review commit \$\{sha\} job in attempt/);
  assert.match(publisher, /selected-commit-review-bundles\.tsv/);
  assert.match(
    publisher,
    /reviewArtifacts\s*\.map\(\(\{ sha, artifact \}\) => `\$\{sha\}\\t\$\{artifact\.name\}\\t\$\{artifact\.attempt\}\\n`\)/,
  );
  assert.match(
    publisher,
  ...
  assert.equal([...publisher.matchAll(/--expected-run-attempt "\$producer_attempt"/g)].length, 2);
  assert.match(publisher, /while IFS=\$'\\t' read -r commit_sha artifact_name producer_attempt;/);
  assert.match(
    publisher,
    /expected_artifact_name="commit-review-\$\{commit_sha\}-\$\{GITHUB_RUN_ID\}-\$\{producer_attempt\}"/,
  );
  assert.match(
    publisher,
    /review_root="\.clawsweeper-repair\/commit-review-download\/\$artifact_name\/ledger"/,
  );
  ...
  assert.ok(
    publisher.indexOf("- name: Attest selected commit review reports") <
      publisher.indexOf("- name: Commit reports"),
  );
});

test("commit report publication binds skipped reports to the latest actual plan attempt", () => {
  const workflow = fs.readFileSync(".github/workflows/commit-review.yml", "utf8");
  const publisher = workflow.slice(workflow.indexOf("\n  publish:"));

  assert.match(publisher, /selectLatestJobResult\(jobInventories, "Plan commits", runAttempt\)/);
  assert.match(publisher, /if \(planJob\.result !== "success"\)/);
  assert.match(publisher, /`skipped-commit-reports-\$\{runId\}`/);
  assert.match(publisher, /successful Plan commits job in attempt/);
  assert.match(
    publisher,
    /name: \$\{\{ steps\.commit-report-cohort\.outputs\.skipped_artifact_name \}\}/,
  );
  assert.match(publisher, /Verify and collect exact skipped commit reports/);
  assert.match(
    publisher,
    /expected_artifact_name="skipped-commit-reports-\$\{GITHUB_RUN_ID\}-\$\{PRODUCER_ATTEMPT\}"/,
  );
  assert.match(publisher, /Skipped commit report artifact count does not match skipped_count/);
  assert.doesNotMatch(
    publisher,
    /name: skipped-commit-reports-\$\{\{ github\.run_id \}\}-\$\{\{ github\.run_attempt \}\}/,
  );
});
