...
artifact comes from the coordinating attempt, while cluster, execute, and
publish jobs are selected independently from the latest attempt in which each
job actually ran. The publisher then requires the matching producer-attempt
ledger for each selected job, verifies repository, SHA, workflow, job, run, and
attempt, and imports the three-lane cohort before mutating durable result state.
Missing, ambiguous, expired, incomplete, extra, or forged required lanes fail
publication. Legacy
