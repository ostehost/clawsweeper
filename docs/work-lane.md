...
The current production lane performs intake, local implementation, validation,
and immutable bundle preparation only. It retains viable code as a deferred
artifact without a target write token, so it does not push a branch, create or
update a PR, or merge.
...
validation, and stops without preparing a bundle when the request is already
fixed, no longer useful, security-sensitive, or needs a maintainer product
decision. Deterministic code still owns repository exclusions, issue identity
and state, security/protected labels, opt-out labels, PR and cluster
deduplication, immutable bundle validation, and allowed no-publication actions.
...
The older strict bug lane remains available and can prepare a publication bundle
only for reviewed issues that are exactly:
...
`CLAWSWEEPER_AUTO_IMPLEMENT_VISION_FIT=1`. It may prepare a publication bundle
only for reviewed issues that are exactly:
...
After a separately trusted publisher exists, PRs created from issue
implementation would be labeled `clawsweeper`,
`clawsweeper:autogenerated`, and `clawsweeper:autofix` and would enter the
...
deferred outcome. A generated PR appears only for historical or future
published work.
