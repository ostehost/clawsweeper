...
reconstructs the minimal `source: issue_implementation` job from the job path for
planning only, instead of treating the dispatch as stale. Fix preparation waits
for the durable job and a later dispatch bound to that exact authoritative job
digest.
