...
- prepares independently validated repair bundles for opted-in PRs through a
  bounded Codex review/fix loop
- prepares guarded implementation bundles for viable reviewed issues in
...
The current production security boundary deliberately stops prepared code at a
hash-bound deferred bundle. Repair and issue-implementation workers do not push
branches or open PRs, and automerge fails closed because GitHub cannot
atomically bind the reviewed base branch at merge time.

For the complete architecture and operator guide covering issue-to-PR work, PR
repair, GitCrawl intake, run-scoped Codex sessions, CrabFleet steering, completion
...
An operator-run Linear sidecar supports workspace, team, project, and exact-item
snapshots; deterministic review-only triage; optional read-only Codex analysis;
and receipt-gated review comments and additive routing labels. It defaults to
dry-run and never closes Linear issues. This is CLI tooling rather than a native
workflow/provider lane; see [`docs/linear-integration.md`](docs/linear-integration.md).
...
exact-head review, and run a bounded Codex review/fix loop. Codex handles local
repair and validation under an isolated principal. Deterministic steps then
validate and stage an exact publication bundle, but the production publisher
does not receive a target write token for prepared code and records the bundle
as deferred.
...
Automerge records maintainer intent and evaluates exact-head review, required
checks, mergeability, and policy gates. It then fails closed at strict base
binding; the current workflow cannot perform the final merge. The mutable
status comment records the review and preparation result, including the
deferred or blocked terminal state.
...
dispatch Codex to prepare one guarded implementation bundle. The current
publisher does not push that bundle or open the intended
`clawsweeper:autogenerated` PR.
...
clear repair shape can use the same deferred implementation path without
...
- `autofix` labels an open PR, creates or reuses the adopted job, dispatches
  review, and enters the bounded review/fix preparation loop. The resulting
  bundle is validated and deferred rather than pushed.
- `automerge` labels an open PR, creates or reuses the adopted job, dispatches
  review, and authorizes the same bounded preparation loop plus merge policy
  evaluation. Strict base binding keeps the final merge disabled. Draft PRs are
  fix-only until GitHub marks them ready for review.
- `implement issue` on an open issue creates or reuses one issue implementation
  job and dispatches the issue-to-bundle lane. OpenClaw organization members
  may request this explicitly even without repository write permission.
- With automatic issue implementation enabled, newly reviewed issues and
  ...
  inspects the issue and repository, chooses the implementation, discovers
  validation, and prepares a deferred bundle when viable. The current workflow
  does not create the intended generated PR or enter its follow-up label/check
  loop.
- User-facing OpenClaw `fix`, `feat`, and `perf` automerge PRs preserve
  release-note context in deferred publication metadata and commit material;
  contributors are not asked to edit `CHANGELOG.md`.
- Security-sensitive findings can be repaired only after explicit
  `autofix`/`automerge` opt-in. Preparation remains non-publishing and cannot
  merge.
- `approve` lets a maintainer clear a ClawSweeper human-review pause and resume
  normal exact-head, checks, mergeability, and preparation gates. Automated
...
- `clawsweeper:human-review` and `clawsweeper:manual-only` stop automatic PR
  repair and issue implementation preparation. Issue implementation rechecks
  the live issue immediately before finalizing its deferred bundle.
...
- apply lane: guarded close/comment mutations, audit, reconcile, and state
  publishing
- repair lane: maintainer-command routing, autofix preparation, automerge
  authorization, issue implementation bundles, and repair result publishing
...
implementation dispatch stays disabled; the bounded broad intake/backfill lane
owns that separately revalidated work. The exact run does not claim an atomic
...
- If review or CI finds actionable issues, Codex rebases, addresses PR review
  ...
  repair artifact.
- The isolated executor applies the artifact in a temporary checkout, validates
  and reviews the exact tree, drains its untrusted principal, and transfers a
  bounded immutable publication bundle.
- The publisher independently validates that bundle but deliberately records it
  as deferred without a target write token. The current production workflow
  therefore does not push repair commits or open replacement/implementation PRs.
- Automated merge is also fail-closed: GitHub cannot atomically bind the
  reviewed base branch, so `automerge` remains an authorization and planning
  intent rather than a live merge path.
- Repair workers coalesce pending runs for the same durable job while allowing
  ...
  window. Before preparing a contributor branch publication, ClawSweeper waits
  90 seconds by default and fetches the live PR head again. A changed head or
  closed PR stops the attempt without mutation; a trusted coordinator or
  operator must use the bounded requeue tooling. Override the window with
  `CLAWSWEEPER_BRANCH_PUSH_SETTLE_SECONDS` (bounded to 0-120 seconds) when a
  manual backfill is already settled.
...
that coalesces item deliveries, leases at most 48 concurrent reviews, and admits
up to 44 active exact reviews per target repository. Other lanes retain the
...
- Review and steerable repair jobs create an isolated run-scoped `CODEX_HOME`.
  No Codex session tree is restored across jobs or attempts. Each job starts a
  local Responses proxy from
  `OPENAI_API_KEY`, writes proxy-only Codex config there, and runs Codex without
  OpenAI or Codex token environment variables.
- Steerable repair jobs expose only the current run-scoped session to
  CrabFleet. CrabFleet credentials stay in the wrapper and are stripped before
  Codex starts.
...
- Contents: read/write, for report commits and repository dispatch inputs that
  need a contents-scoped installation token. The production repair publisher
  does not use this permission to push prepared code.
- Issues: read/write, for issue comments, labels, closes, and maintainer command
  authorization context.
- Pull requests: read/write, for PR comments, labels, merge readiness, and
  command status. The production workflow does not create or merge repair PRs.
...
- variable `CLAWSWEEPER_STEERABLE_CODEX=1`: enables run-scoped app-server
  sessions and browser steering in the repair cluster workflow
- variable `CLAWSWEEPER_CRABFLEET_URL`: optional CrabFleet API/dashboard base;
  defaults to `https://crabfleet.openclaw.ai`

See
[`docs/steerable-repair-automation.md`](docs/steerable-repair-automation.md)
for the registration, token, heartbeat, run-scoped steering, completion,
